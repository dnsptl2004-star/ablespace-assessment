...
export const Sidebar: React.FC<SidebarProps> = ({ mobileOpen = false, onCloseMobile }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar Aside Panel */}
      <aside
        className={`w-64 fixed inset-y-0 left-0 z-50 flex flex-col glass-panel border-r border-slate-200 dark:border-slate-800 transition-transform duration-300 ease-in-out lg:translate-x-0 ${
          mobileOpen ? 'translate-x-0' : '-translate-x-full'
        }`}
      >
        {/* Brand Header */}
        <div className="h-16 sm:h-20 flex items-center justify-between px-6 border-b border-slate-200/80 dark:border-slate-800/80">
          <Link
            href="/dashboard"
            onClick={onCloseMobile}
          ...
          <button
            onClick={onCloseMobile}
            className="lg:hidden p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 rounded-lg hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors"
