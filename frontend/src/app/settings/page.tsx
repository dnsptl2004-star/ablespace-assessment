...
import { useTheme, ACCENT_COLOR_MAP } from '../../context/ThemeContext';
import { ThemeMode, AccentColor } from '../../types';
import { api } from '../../lib/api';
import { Sun, Moon, Laptop, Palette, Check, Save, Loader2 } from 'lucide-react';

export default function SettingsPage() {
  const { themeMode, accentColor, compactMode, setThemeMode, setAccentColor, setCompactMode } = useTheme();
        ...
        <main className="p-8 max-w-4xl mx-auto space-y-8">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-3 pt-2">
              {colorOptions.map((opt) => {
                ...
                return (
                  ...
                  >
                    <span className={`w-4 h-4 rounded-full ${opt.bgClass}`} />
                    <span className="text-xs text-slate-800 dark:text-slate-200">{opt.label.split(' ')[0]}</span>
          ...
          <div className="flex justify-end pt-4">
            <button
              ...
              disabled={isSaving}
              className="flex items-center gap-2 px-6 py-3 rounded-2xl bg-indigo-600 hover:bg-indigo-700 dark:bg-indigo-500 dark:hover:bg-indigo-600 text-white font-bold text-xs shadow-lg shadow-indigo-500/25 disabled:opacity-50 transition-all"
            >
              {isSaving ? (
